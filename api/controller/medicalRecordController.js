...
//   try {
//     res.sendFile(
//       `${path.resolve()}\\public\\medicalRecords\\${req.query.id}\\${
...
async function historyTodatabase(recordObj, next) {
  ...
  var query = `MATCH(n:Patient{value:"${params.subject.identifier.value}"})
      ...
      MERGE(m)-[:category]->(:coding{system:"${params.category[0].coding[0].system}",code:"${params.category[0].coding[0].code}"})
      ...
      Merge(a)-[:event{text:"${params.context.event[0].text}"}]->(:coding{system:"${params.context.event[0].coding[0].system}",value:"${params.context.event[0].coding[0].value}"})
...
addRecord("20000101-633940", next);
