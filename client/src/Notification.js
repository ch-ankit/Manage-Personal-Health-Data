    ...
    if (timeNumber.current.value > 0) {
      ...
      if (timeNumber !== null && timeUnit !== null) {
        ...
        const response = await fetch(
          `http://localhost:7000/personal/requesteddocument`,
          {
            method: "POST",
            headers: {
              "Content-type": "application/json",
        ...
        if (decision === "granted") {
          document
            .querySelector(".notification__sharePopup.active")
    ...
    displayNotifications = Object.keys(notification).map((el) => {
      return (
        <Link
          ...
          key={el}
          to="/Doctor/documentViewer"
        >
          <div
            className={`${
              notification[el].markAsRead === "false"
                ? "notification__display"
                : "notification__displayRed"
            }`}
          >
            <div style={{ width: "3em", height: "3em" }}>
              <img
                src={notification[el].photo}
                alt="Patient DP"
                className={`notification__patientPhoto  ${
                  darkMode && "notification__imgDark"
                }`}
              />
            </div>
            {notification[el].patientName} has shared a {notification[el].title}{" "}
            record with you
          </div>
        </Link>
      );
        ...
        <div className="friendlist__requests">
          <div style={{ height: "7em", width: "7em" }}>
            <img
              ...
              className={`friendlist__patientPhoto  ${
                darkMode && "friendlist__imgDark"
              }`}
              alt="Doctor Dp"
            />
          </div>
            ...
            >
              <button
                ...
                onClick={() => {
                  ...
                  document
                    .querySelector(".notification__sharePopup")
                    .classList.toggle("active");
                }}
              >
                <span style={{ color: "black" }}>Accept</span>
                <CheckIcon className="friendlist__userIconAdd" />
              </button>
              <button
                style={{
                  display: "flex",
                ...
                onClick={() =>
                  userDecision(
                    notification[el].doctorId,
                    "rejected",
                    notification[el].masterId
                  )
                }
              >
                <span style={{ color: "black" }}>Reject</span>
                <XIcon className="friendlist__userRemoveIcon" />
              </button>
            </div>
          </div>
        </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <form
          className="notification__sharePopup"
          onSubmit={(e) => {
            e.preventDefault();
            userDecision(tempData.doctorId, tempData.status, tempData.masterId);
          }}
        >
          <h2>Share</h2>
